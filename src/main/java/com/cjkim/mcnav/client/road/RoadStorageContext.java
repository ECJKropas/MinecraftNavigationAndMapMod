...
public final class RoadStorageContext {
    private static final RoadStorageContext UNBOUND = new RoadStorageContext("未进入世界", "unbound", false);
    ...
    public static RoadStorageContext resolve(Minecraft client) {
        ...
        if (singleplayerServer != null) {
            ...
            String source = "singleplayer|"
                + normalizeSource(worldPath == null ? "unknown" : worldPath.toAbsolutePath().normalize().toString());
            return new RoadStorageContext("单人存档 · " + worldName,
                "singleplayer-" + slugify(worldName) + "-" + shortHash(source), true);
        }

        var currentServer = client.getCurrentServer();
        if (currentServer != null) {
            String address =
                currentServer.ip == null || currentServer.ip.isBlank() ? "unknown" : currentServer.ip.trim();
            String serverName =
                currentServer.name == null || currentServer.name.isBlank() ? address : currentServer.name.trim();
            String prefix = currentServer.isLan() ? "本地服务器" : "联机服务器";
            String source = "server|" + normalizeSource(address);
            return new RoadStorageContext(prefix + " · " + serverName,
                "server-" + slugify(serverName) + "-" + shortHash(source), true);
        }
    ...
    private static String slugify(String input) {
        ...
        String slug = normalized.replaceAll("[^a-z0-9]+", "-").replaceAll("^-+", "").replaceAll("-+$", "");
