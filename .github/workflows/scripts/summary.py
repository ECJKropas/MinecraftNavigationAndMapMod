...
def main():
    ...
    workflow_artifacts_json_str = os.environ.get(
    ...
    try:
        artifacts = {
            ...
            for artifact in json.loads(workflow_artifacts_json_str)["artifacts"]
        }
        print({artifact["name"]: artifact["id"] for artifact in artifacts.values()})
    except Exception as e:
        warnings.append(
            f"Failed to parse workflow artifacts JSON: {e}, {workflow_artifacts_json_str!r}"
        )
    ...
    with open(os.environ["GITHUB_STEP_SUMMARY"], "w") as f:
        ...
        for subproject in settings["versions"]:
            ...
            game_versions = read_prop(
                f"versions/{subproject}/gradle.properties", "gameVersions"
            )
            game_versions = game_versions.strip().replace("\r", "").replace("\n", ", ")
            file_paths = glob.glob(
                "build-artifacts/{}/build/libs/*.jar".format(subproject)
            )
            ...
            file_paths = [
                ...
                if all(
                    not fp.endswith(f"-{classifier}.jar")
                    for classifier in ["sources", "dev", "shadow"]
                )
            ]
            if len(file_paths) == 0:
                file_name = "*not found*"
                ...
                if len(file_paths) > 1:
                    warnings.append(
                        "Found too many build files in subproject {}: {}".format(
                            subproject, ", ".join(file_paths)
                        )
                    )

            f.write(
                f"| {subproject} | {game_versions} | {file_name} | {file_size} | {sha256} |\n"
            )
        f.write("\n")

        f.write("## Artifact Files\n\n")
        all_digests_are_sha256 = all(
            artifact["digest"].startswith("sha256:") for artifact in artifacts.values()
        )
        f.write(
            "| Artifact | For | Size | {} | \n".format(
                "SHA-256" if all_digests_are_sha256 else "Digest"
            )
        )
        f.write("| --- | --- | --- | --- |\n")
        for artifact_name, artifact_usage in [
            (
                "mod-jars",
                "Players who want to grab and install the mod jar into their Minecraft clients",
            ),
            (
                "build-artifacts",
                "Mod developers who want to inspect the complete build artifacts",
            ),
        ]:
            ...
            if artifact_name in artifacts:
                ...
                try:
                    ...
                    artifact_digest = "`{}`".format(
                        ...
                        if all_digests_are_sha256
                        else artifact["digest"]
                    )
                except Exception as e:
                    warnings.append(
                        f"Failed to collect artifact info for {artifact_name}: {e} -- {artifact}"
                    )

            f.write(
                f"| {artifact_display_name} | {artifact_usage} | {artifact_size} | {artifact_digest} |\n"
            )
        f.write("\n")
